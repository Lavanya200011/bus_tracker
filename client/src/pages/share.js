...
import { Play, Square, Search } from 'lucide-react'; // Search आइकन इम्पोर्ट किया
  ...
  return (
    <div className="p-6 max-w-lg mx-auto font-sans">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-extrabold text-gray-900 tracking-tighter">Share Journey</h2>
          <p className="text-gray-500 text-sm">Help others track this bus.</p>
        </div>
        <Link href="/search">
          <button className="p-3 bg-gray-100 text-gray-600 rounded-2xl hover:bg-blue-100 hover:text-blue-600 transition-all active:scale-90 shadow-sm border border-gray-200">
            <Search size={24} />
          </button>
        </Link>
      </div>
      ...
      <div className="space-y-4 mb-8 text-left">
        ...
        <select 
          className="w-full p-4 border-2 border-gray-100 rounded-2xl bg-white shadow-sm outline-none focus:border-blue-500 transition-all text-gray-700 font-semibold cursor-pointer"
      ...
      <div className={`p-8 rounded-[3rem] text-center transition-all duration-500 border-4 ${isTracking ? 'bg-red-50 border-red-100 shadow-red-50' : 'bg-blue-50 border-blue-100 shadow-blue-50'} shadow-2xl`}>
        <div className="mb-6 flex justify-center">
            <div className={`p-4 rounded-full shadow-lg ${isTracking ? 'bg-red-500 animate-pulse' : 'bg-blue-600 text-white'}`}>
                {isTracking ? <Square className="text-white" size={24}/> : <Play className="text-white ml-1" size={24}/>}
            </div>
        </div>

        <p className={`mb-6 font-black tracking-widest text-sm ${isTracking ? 'text-red-600' : 'text-blue-600'}`}>
          {isTracking ? "📡 BROADCASTING LIVE" : "READY TO BROADCAST"}
        </p>
        
        <button 
          onClick={toggleTracking}
          className={`w-full py-4 rounded-2xl font-black text-white uppercase tracking-widest transition-all shadow-xl active:scale-95 ${
            isTracking 
      ...
      <p className="mt-8 text-center text-xs text-gray-400 italic font-medium">
        {isTracking 
          ...
          : "Start sharing to help fellow commuters."}
      </p>
    </div>
  );
}